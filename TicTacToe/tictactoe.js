...
// Function to disable the difficulty radio buttons
function disableDifficultySelection() {
  const radios = document.getElementsByName("difficulty");
  radios.forEach((radio) => {
    radio.disabled = true; // Disable radio buttons to prevent changes
  });
}

// Function to enable the difficulty radio buttons
...
function togglePlayer() {
  ...
  if (currentPlayer === "O" && gameActive) {
    ...
    if (difficulty === "easy") randomComputerMove();
    else if (difficulty === "medium") staticEvaluatorComputerMove();
    else minimaxComputerMove(9);
  }
}
